...
# ==============================================================================
# Clés ici en dur ou via des variables d'environnement
...
FILE_BILLBOARD_RAW = 'billboard_raw.csv'         # Intermédiaire partie 2
...
if GEMINI_API_KEY.startswith("TA_CLE"):
    print("⚠️ ATTENTION : Pense à configurer ta clé GEMINI_API_KEY")
else:
...
# ==============================================================================
# PARTIE 1 : ETL SPOTIFY (Ton code original amélioré)
# ==============================================================================
...
def get_spotify_token():
    if SPOTIFY_CLIENT_ID.startswith("TA_CLE"):
        print("❌ Erreur : Clés Spotify non configurées.")
        return None
...
def fetch_tracks_metadata(track_ids, token):
    ...
    try:
        ...
        print(f"⚠️ Erreur Batch Spotify : {e}")
    return metadata_map
...
def run_spotify_etl():
    print("\n--- 🟢 1. DÉMARRAGE ETL SPOTIFY (2000-2023) ---")
    if os.path.exists(FILE_SPOTIFY_JSON):
        print(f"✅ Le fichier {FILE_SPOTIFY_JSON} existe déjà. On passe.")
        return

    # Extract Kaggle
    print("📥 Téléchargement Dataset Kaggle...")
    ...
    print(f"📊 {len(df_hits)} chansons à traiter.")
    ...
    print("🌍 Récupération métadonnées API...")
    ...
    print(f"✅ ETL Spotify terminé : {FILE_SPOTIFY_JSON}")
...
def scrape_billboard(start_year, end_year):
    ...
    print(f"🕷️ Scraping Wikipedia de {start_year} à {end_year}...")
    for year in range(start_year, end_year + 1):
        try:
            ...
            for row in target.find_all('tr'):
                ...
                # Logique simplifiée : souvent col 1 = titre, col 2 = artiste (si rang en col 0)
                if len(texts) >= 3:
...
def enrich_gemini(df_raw):
    print("🤖 Enrichissement via Gemini...")
    ...
    if not os.path.exists(FILE_BILLBOARD_ENRICHED):
        ...
        print(f"🔄 Reprise à la ligne {start_idx}")
    ...
    for i in tqdm(range(start_idx, len(records), batch_size)):
        ...
        while not success:
            ...
            try:
                ...
                if "429" in str(e):
                    print("⚠️ Changement de modèle Gemini...")
                    current_model += 1
                else:
...
def run_billboard_etl():
    print("\n--- 🟡 2. DÉMARRAGE ETL BILLBOARD (1980-2000) ---")
    ...
    if os.path.exists(FILE_BILLBOARD_ENRICHED) and len(pd.read_csv(FILE_BILLBOARD_ENRICHED)) >= len(df):
        print("✅ Enrichissement déjà terminé.")
    else:
        enrich_gemini(df)
...
def run_fusion():
    print("\n--- 🔵 3. DÉMARRAGE FUSION FINALE ---")
    
    # Load Spotify
    if not os.path.exists(FILE_SPOTIFY_JSON): return print("❌ Manque fichier Spotify")
    df_spotify = pd.read_json(FILE_SPOTIFY_JSON)
    df_spotify['year'] = pd.to_numeric(df_spotify['year'], errors='coerce')
    df_spotify = df_spotify[df_spotify['year'] >= 2000] # On garde le récent
    if 'duration_fmt' in df_spotify.columns:
        df_spotify = df_spotify.rename(columns={'duration_fmt': 'duration'})

    # Load Billboard
    if not os.path.exists(FILE_BILLBOARD_ENRICHED): return print("❌ Manque fichier Billboard")
    ...
    print(f"🎉 FUSION TERMINÉE ! Fichier généré : {FILE_FINAL_MERGED}")
